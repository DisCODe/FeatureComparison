...
void CvFlann::onNewImage()
{
	...
	try {
		...
		// Matching descriptor vectors using FLANN matcher.
                // Matcher adjusted to match binary descriptors, with default,
		...
		if (distance_recalc) {
			...
			for( int i = 0; i < descriptors_1.rows; i++ )
			{
				...
				if( dist > max_dist ) max_dist = dist;
                        ...
                        // This line was disabled, because in binary descriptors Hamming distance
                        // is being used. This sort of calculation can give very misleading results.
			//dist = 2*min_dist;
		...
		for( int i = 0; i < descriptors_1.rows; i++ )
		{
                        // dist is a constant value (defalut it is 64)
